...
function isMobileRuntime() {
  ...
  const hasTouch = Number(navigator.maxTouchPoints || navigator.msMaxTouchPoints || 0) > 0 ||
  ...
  return isMobileUserAgent || hasCoarsePointer || hasAnyCoarsePointer || (hasTouch && isSmallTouchScreen)
}
...
function isCompactPlaybackControlContext() {
  const viewport = window.visualViewport
  ...
  return isMobileRuntime() || isCompactPlayerViewport() || (Number.isFinite(shortSide) && shortSide > 0 && shortSide <= 980)
}
...
const MOBILE_CORE_CONTROL_SELECTOR = [
  ...
  '.art-control-mobile-danmu-settings-trigger',
  '.art-control-mobile-quality',
  '.art-control-mobile-rate',
  '.art-control-mobile-subtitle',
  '.art-control-画质',
  '.art-control-倍速',
  '.art-control-字幕',
  '.art-control-setting',
...
function createMobileCoreControlDefinitions() {
  return [
    {
    ...
    },
    {
      ...
      index: 0.3,
      position: 'right',
      html: '<span class="mobile-art-text-control mobile-art-quality-label">画质</span>',
      tooltip: '画质',
      click: function () {
        toggleMobileControlMenu('quality')
      }
    },
    {
      name: 'mobile-rate',
      index: 0.4,
      position: 'right',
      html: '<span class="mobile-art-text-control mobile-art-rate-label">1X</span>',
      tooltip: '倍速',
      click: function () {
        toggleMobileControlMenu('rate')
      }
    },
    {
      ...
      index: 0.5,
      position: 'right',
      html: '<span class="mobile-art-text-control mobile-art-subtitle-label">字幕</span>',
      tooltip: '字幕',
      click: function () {
        toggleMobileControlMenu('subtitle')
      }
    },
    {
...
const setting = ref({
  ...
  settings: [],
  controls: [
    ...createMobileCoreControlDefinitions(),
    // {
    //     position: 'right',
    //     index: 15,
...
function isMobileUiActive() {
  return mobileUiActive.value || MOBILE_UA || isMobileRuntime() || isCompactPlayerViewport() || isForcedLandscapeActive()
}
...
function shouldSkipMobileDanmuSettingsActivator(event, now) {
  const type = event?.type || ''
  if (!event) {
    return now - lastMobileDanmuSettingsToggleAt < MOBILE_DANMU_SETTINGS_RELEASE_DEDUPE_MS
  }
  if (now < ignoreMobileDanmuSettingsReleaseUntil && !isMobileDanmuTriggerPress(event)) {
    return true
  }
  ...
  if (type === 'touchend') {
    return lastMobileDanmuSettingsPointerUpAt > lastMobileDanmuSettingsTouchAt &&
        now - lastMobileDanmuSettingsPointerUpAt < MOBILE_DANMU_SETTINGS_RELEASE_DEDUPE_MS
  }
  if (type === 'pointerup' && event.pointerType !== 'mouse') {
    return lastMobileDanmuSettingsTouchEndAt > lastMobileDanmuSettingsTouchAt &&
...
function toggleMobileDanmuSettingsOnPress(event) {
  const now = window.performance?.now?.() || Date.now()
  if (markMobileDanmuTriggerEvent(event)) {
    preventMobileDanmuTriggerEvent(event)
    return
  }
  if (now - lastMobileDanmuSettingsTouchAt < MOBILE_DANMU_SETTINGS_PRESS_DEDUPE_MS) {
    preventMobileDanmuTriggerEvent(event)
    return
  }
  lastMobileDanmuSettingsTouchAt = now
  ...
  mobileDanmuSettingsPressStarted = false
  mobileDanmuSettingsPressCaptured = true
  preventMobileDanmuTriggerEvent(event)
  toggleMobileDanmuSettingsPanelState()
}

function absorbMobileDanmuSettingsRelease(event) {
  if (!mobileDanmuSettingsPressCaptured) {
    return false
  }
  const now = window.performance?.now?.() || Date.now()
  if (now > ignoreMobileDanmuSettingsReleaseUntil) {
    mobileDanmuSettingsPressCaptured = false
    return false
  }
  if (isMobileDanmuTriggerActivator(event) || (event?.type || '') === 'click') {
    preventMobileDanmuTriggerEvent(event)
    return true
  }
  return false
...
function shouldIgnoreMobileDanmuSettingsFollowUpClose(event) {
  if ((event?.type || '') !== 'click') {
    return false
  }
  const now = window.performance?.now?.() || Date.now()
...
function handleDirectMobileDanmuPanelTrigger(event) {
  ...
  if (absorbMobileDanmuSettingsRelease(event)) {
    return
  }
  if (isMobileDanmuTriggerPress(event)) {
    ...
    if (shouldToggleOnPress) {
      toggleMobileDanmuSettingsOnPress(event)
    } else {
...
function bindMobileDanmuPanelTriggerElement(trigger) {
  ...
  if (window.PointerEvent) {
    ...
    trigger.addEventListener('pointerdown', handleDirectMobileDanmuPanelTrigger)
    trigger.addEventListener('pointerup', handleDirectMobileDanmuPanelTrigger)
  }
  ...
  trigger.addEventListener('mousedown', handleDirectMobileDanmuPanelTrigger)
  trigger.addEventListener('mouseup', handleDirectMobileDanmuPanelTrigger)
  ...
  trigger.addEventListener('touchstart', handleDirectMobileDanmuPanelTrigger, {passive: false})
  trigger.addEventListener('touchend', handleDirectMobileDanmuPanelTrigger, {passive: false})
  trigger.addEventListener('click', handleDirectMobileDanmuPanelTrigger, {capture: true})
  trigger.addEventListener('click', handleDirectMobileDanmuPanelTrigger)
}
...
function createMobileFallbackCoreControl(control) {
  ...
  const existingControl = root.querySelector(selector)
  if (existingControl) {
    if (!rightControls.contains(existingControl)) {
      rightControls.appendChild(existingControl)
    }
    existingControl.classList.remove('art-control-hide')
    existingControl.style.setProperty('display', 'inline-flex', 'important')
    existingControl.style.setProperty('visibility', 'visible', 'important')
    existingControl.style.setProperty('opacity', '1', 'important')
    existingControl.style.setProperty('pointer-events', 'auto', 'important')
    existingControl.style.setProperty('touch-action', 'manipulation', 'important')
    if (control.name === 'mobile-danmu-settings-trigger') {
      bindMobileDanmuPanelTriggerElement(existingControl)
    }
    return
  }
  ...
  button.addEventListener('pointerdown', event => {
    if (control.name === 'mobile-danmu-settings-trigger') {
      handleDirectMobileDanmuPanelTrigger(event)
    }
  }, {capture: true})
...
function areMobileTextControlsVisible() {
  const requiredControls = ['.art-control-mobile-rate']
  if (qualitySelector.value.length > 0) {
    requiredControls.push('.art-control-mobile-quality')
  }
  if (StreamList.value?.subtitle_streams?.length > 0) {
    requiredControls.push('.art-control-mobile-subtitle')
  }
  return requiredControls.every(selector => isMobileArtControlVisible(selector))
}

function syncMobileInlineArtControlLabels(root = playerFrame.value) {
  if (!root) {
    return
  }
  const setLabel = (selector, label) => {
    root.querySelectorAll(selector).forEach(control => {
      ...
      const labelNode = control.querySelector('.mobile-art-text-control') || control
      labelNode.textContent = text
      control.setAttribute('title', text)
      control.setAttribute('aria-label', text)
    })
  }
  setLabel('.art-control-mobile-quality', mobileQualityLabel.value || '画质')
  setLabel('.art-control-mobile-rate', `${Number(mobilePlaybackRate.value) || 1}X`)
  setLabel('.art-control-mobile-subtitle', mobileSubtitleLabel.value || '字幕')
}

function ensureMobileCoreControls() {
  const root = playerFrame.value
  if (!root || !art?.controls || !isMobileUiActive()) {
    return
  }
  createMobileCoreControlDefinitions().forEach(control => {
    const selector = `.art-control-${control.name}`
    const existingControl = root.querySelector(selector)
    if (existingControl && isMobileArtControlVisible(selector)) {
      return
    }
    try {
      art.controls.remove(control.name)
    } catch {
    }
    try {
    ...
    if (!root.querySelector(selector) || !isMobileArtControlVisible(selector)) {
...
function normalizeMobileCoreControls() {
  ...
  root.querySelectorAll('.art-control-画质, .art-control-倍速, .art-control-字幕').forEach(button => {
    button.classList.add('art-control-hide')
    button.style.setProperty('display', 'none', 'important')
    button.style.setProperty('visibility', 'hidden', 'important')
    button.style.setProperty('opacity', '0', 'important')
  ...
  root.querySelectorAll('.art-control-mobile-danmu-toggle span, .art-control-mobile-danmu-settings-trigger span, .art-control-mobile-danmu-settings-trigger i, .mobile-art-text-control').forEach(child => {
    child.style.pointerEvents = 'none'
  })
  syncMobileInlineArtControlLabels(root)
  normalizeMobileCoreControlRow(root)
}

function normalizeMobileCoreControlRow(root = playerFrame.value) {
  if (!root || !isMobileUiActive()) {
    return
  }
  const rightControls = root.querySelector('.art-video-player .art-controls-right')
  ...
  orderedSelectors.forEach((selector, index) => {
    root.querySelectorAll(selector).forEach(control => {
      if (!rightControls.contains(control) && control.parentElement) {
        rightControls.appendChild(control)
      }
      control.style.setProperty('order', String(10 + index), 'important')
  ...
  if (isPortraitMobilePlayer()) {
    ...
    const widths = {
      '.art-control-mobile-danmu-toggle': narrow ? 21 : 23,
      '.art-control-mobile-danmu-settings-trigger': narrow ? 30 : 32,
      '.art-control-mobile-quality': narrow ? 32 : 34,
      '.art-control-mobile-rate': narrow ? 21 : 23,
      '.art-control-mobile-subtitle': narrow ? 26 : 28,
      '.art-control-setting': narrow ? 21 : 23,
      '.art-control-mobile-landscape-fullscreen': narrow ? 21 : 23
    }
    const rightWidth = Object.values(widths).reduce((sum, width) => sum + width, 0)
    const leftMin = narrow ? 86 : 94
    const leftMax = narrow ? 108 : 118
    const leftWidth = Math.round(Math.max(leftMin, Math.min(leftMax, viewportWidth - rightWidth - 8)))
    ...
    setImportantStyle(rightControls, 'overflow', 'visible')
    setImportantStyle(rightControls, 'overflow-x', 'visible')
    setImportantStyle(rightControls, 'overflow-y', 'visible')
  ...
  if (root.querySelector('.art-control-mobile-landscape-fullscreen')) {
    root.querySelectorAll('.art-control-fullscreen, .art-control-fullscreenWeb').forEach(control => {
      control.classList.add('art-control-hide')
...
function handleMobileDanmuPanelClick(event) {
  ...
  if (!root || !target || !root.contains(target)) {
    if (event?.type === 'click') {
      if (shouldIgnoreMobileDanmuSettingsFollowUpClose(event)) {
        preventMobileDanmuTriggerEvent(event)
        return
      }
      closeMobileDanmuPanels()
...
async function selectMobileQuality(option) {
  ...
  syncMobileInlineArtControlLabels()
  mobileControlMenu.value = ''
}
...
async function switchQuality(item, $dom, event) {
  ...
  try {
    ...
    if (res.updateM3u8) {
      ...
      return isCompactPlaybackControlContext() ? qualityData.resolution : `${qualityData.resolution} (${(qualityData.bitrate / 1000000).toFixed(1)}Mbps)`;
  ...
  if (qualitySelector.value.length > 0) {
    const qualityControl = {
      ...
      html: currentQuality.value ?
          (isCompactPlaybackControlContext() ? currentQuality.value.resolution : `${currentQuality.value.resolution} (${(currentQuality.value.bitrate / 1000000).toFixed(1)}Mbps)`) :
          '画质',
...
:deep(.art-video-player .art-control-mobile-quality),
:deep(.art-video-player .art-control-mobile-rate),
:deep(.art-video-player .art-control-mobile-subtitle) {
  position: relative;
  z-index: 30;
  display: none !important;
  align-items: center;
...
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-quality),
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-rate),
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-subtitle) {
  display: flex !important;
  visibility: visible !important;
  opacity: 1 !important;
  pointer-events: auto !important;
...
/* Final portrait row contract: same core controls as landscape, starting from danmu. */
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) {
  --mobile-control-left-complete: clamp(96px, 30vw, 122px);
  --mobile-control-danmu-complete: 28px;
  --mobile-control-danmu-settings-complete: 38px;
  --mobile-control-quality-complete: 38px;
  --mobile-control-rate-complete: 28px;
  --mobile-control-subtitle-complete: 34px;
  --mobile-control-icon-complete: 26px;
}
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-controls-left) {
  flex: 0 0 var(--mobile-control-left-complete) !important;
  width: var(--mobile-control-left-complete) !important;
  min-width: 0 !important;
  max-width: var(--mobile-control-left-complete) !important;
  overflow: hidden !important;
}
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
  ...
  gap: 0 !important;
  min-width: 0 !important;
  max-width: calc(100% - var(--mobile-control-left-complete) - 8px) !important;
  overflow-x: auto !important;
  overflow-y: hidden !important;
  scrollbar-width: none !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-toggle) {
  order: 10 !important;
  flex-basis: var(--mobile-control-danmu-complete) !important;
  width: var(--mobile-control-danmu-complete) !important;
  min-width: var(--mobile-control-danmu-complete) !important;
  max-width: var(--mobile-control-danmu-complete) !important;
}

.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-settings-trigger) {
  order: 11 !important;
  flex-basis: var(--mobile-control-danmu-settings-complete) !important;
  width: var(--mobile-control-danmu-settings-complete) !important;
  min-width: var(--mobile-control-danmu-settings-complete) !important;
  max-width: var(--mobile-control-danmu-settings-complete) !important;
}

.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-画质) {
  order: 12 !important;
  flex-basis: var(--mobile-control-quality-complete) !important;
  width: var(--mobile-control-quality-complete) !important;
  min-width: var(--mobile-control-quality-complete) !important;
  max-width: var(--mobile-control-quality-complete) !important;
}

.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-倍速) {
  order: 13 !important;
  flex-basis: var(--mobile-control-rate-complete) !important;
  width: var(--mobile-control-rate-complete) !important;
  min-width: var(--mobile-control-rate-complete) !important;
  max-width: var(--mobile-control-rate-complete) !important;
}

.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-字幕) {
  order: 14 !important;
  flex-basis: var(--mobile-control-subtitle-complete) !important;
  width: var(--mobile-control-subtitle-complete) !important;
  min-width: var(--mobile-control-subtitle-complete) !important;
  max-width: var(--mobile-control-subtitle-complete) !important;
}

.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-setting) {
  order: 15 !important;
  flex-basis: var(--mobile-control-icon-complete) !important;
  width: var(--mobile-control-icon-complete) !important;
  min-width: var(--mobile-control-icon-complete) !important;
  max-width: var(--mobile-control-icon-complete) !important;
}
...
@media (max-width: 360px) and (orientation: portrait) {
  .player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) {
    --mobile-control-left-complete: clamp(88px, 28vw, 104px);
    --mobile-control-danmu-complete: 26px;
    --mobile-control-danmu-settings-complete: 36px;
    --mobile-control-quality-complete: 34px;
    --mobile-control-rate-complete: 26px;
    --mobile-control-subtitle-complete: 32px;
    --mobile-control-icon-complete: 24px;
  }
}

/* Real-phone portrait row: keep the same core controls visible as landscape. */
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) {
  --mobile-phone-left-controls: clamp(92px, 27vw, 108px);
  --mobile-phone-danmu: 24px;
  --mobile-phone-danmu-settings: 34px;
  --mobile-phone-quality: 34px;
  --mobile-phone-rate: 24px;
  --mobile-phone-subtitle: 30px;
  --mobile-phone-icon: 24px;
}
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-toggle) {
  order: 10 !important;
  flex-basis: var(--mobile-phone-danmu) !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-settings-trigger) {
  order: 11 !important;
  flex-basis: var(--mobile-phone-danmu-settings) !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-画质) {
  order: 12 !important;
  flex-basis: var(--mobile-phone-quality) !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-倍速) {
  order: 13 !important;
  flex-basis: var(--mobile-phone-rate) !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-字幕) {
  order: 14 !important;
  flex-basis: var(--mobile-phone-subtitle) !important;
...
.player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) :deep(.art-video-player .art-control-setting) {
  order: 15 !important;
  flex-basis: var(--mobile-phone-icon) !important;
...
@media (max-width: 360px) and (orientation: portrait) {
  .player.is-mobile-player.is-mobile-portrait:not(.is-forced-landscape) {
    --mobile-phone-left-controls: clamp(82px, 26vw, 94px);
    --mobile-phone-danmu: 22px;
    --mobile-phone-danmu-settings: 32px;
    --mobile-phone-quality: 32px;
...
.player.is-mobile-player:not(.is-forced-landscape) {
  --mobile-final-left-controls: clamp(104px, 28vw, 118px);
  --mobile-final-danmu: 28px;
  --mobile-final-danmu-settings: 40px;
  --mobile-final-quality: 42px;
  --mobile-final-rate: 28px;
  --mobile-final-subtitle: 36px;
  --mobile-final-icon: 28px;
}
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-left) {
  display: flex !important;
  flex: 0 0 var(--mobile-final-left-controls) !important;
  width: var(--mobile-final-left-controls) !important;
  min-width: 0 !important;
  max-width: var(--mobile-final-left-controls) !important;
  overflow: hidden !important;
}
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-time) {
  ...
  width: auto !important;
  min-width: 0 !important;
  max-width: calc(var(--mobile-final-left-controls) - 48px) !important;
  padding-inline: 0 !important;
  overflow: hidden !important;
  font-size: 10px !important;
  line-height: 40px !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
  ...
  gap: 0 !important;
  min-width: 0 !important;
  max-width: calc(100% - var(--mobile-final-left-controls) - 8px) !important;
  overflow: visible !important;
  transform: none !important;
  scrollbar-width: none !important;
}
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-toggle) {
  order: 10 !important;
  flex-basis: var(--mobile-final-danmu) !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-settings-trigger) {
  order: 11 !important;
  flex-basis: var(--mobile-final-danmu-settings) !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-画质) {
  order: 12 !important;
  flex-basis: var(--mobile-final-quality) !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-倍速) {
  order: 13 !important;
  flex-basis: var(--mobile-final-rate) !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-字幕) {
  order: 14 !important;
  flex-basis: var(--mobile-final-subtitle) !important;
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-setting),
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-landscape-fullscreen) {
  flex-basis: var(--mobile-final-icon) !important;
...
@media (max-width: 360px) and (orientation: portrait) {
  .player.is-mobile-player:not(.is-forced-landscape) {
    --mobile-final-left-controls: clamp(88px, 25vw, 100px);
    --mobile-final-danmu: 24px;
    --mobile-final-danmu-settings: 34px;
    --mobile-final-quality: 38px;
    --mobile-final-rate: 24px;
    --mobile-final-subtitle: 32px;
    --mobile-final-icon: 24px;
  }
}

/* True-phone fallback: keep the same bottom controls even if the browser reports a desktop-like UA. */
.player :deep(.art-video-player .art-control-mobile-danmu-settings-trigger),
.player :deep(.art-video-player .artplayer-plugin-danmuku .apd-config),
.player :deep(.art-video-player .artplayer-plugin-danmuku [class*="apd-config"]) {
  position: relative !important;
...
.player :deep(.art-video-player .art-control-mobile-danmu-settings-trigger)::before,
.player :deep(.art-video-player .artplayer-plugin-danmuku .apd-config)::before,
.player :deep(.art-video-player .artplayer-plugin-danmuku [class*="apd-config"])::before {
  position: absolute;
  inset: -14px -10px;
  content: "";
}

.player .mobile-danmu-settings.is-visible {
  display: block !important;
  visibility: visible !important;
  opacity: 1 !important;
  pointer-events: auto !important;
...
@media (max-width: 640px), (pointer: coarse), (hover: none) {
  .player:not(.is-forced-landscape) {
    --mobile-fallback-left-controls: clamp(96px, 28vw, 112px);
    --mobile-fallback-danmu: 26px;
    --mobile-fallback-danmu-settings: 38px;
    --mobile-fallback-quality: 40px;
    --mobile-fallback-rate: 26px;
    --mobile-fallback-subtitle: 34px;
    --mobile-fallback-icon: 26px;
  }
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-left) {
    display: flex !important;
    flex: 0 0 var(--mobile-fallback-left-controls) !important;
    width: var(--mobile-fallback-left-controls) !important;
    min-width: 0 !important;
    max-width: var(--mobile-fallback-left-controls) !important;
    overflow: hidden !important;
  }
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-time) {
    ...
    width: auto !important;
    min-width: 0 !important;
    max-width: calc(var(--mobile-fallback-left-controls) - 44px) !important;
    padding-inline: 0 !important;
    overflow: hidden !important;
    font-size: 10px !important;
    line-height: 40px !important;
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
    ...
    gap: 0 !important;
    min-width: 0 !important;
    max-width: calc(100% - var(--mobile-fallback-left-controls) - 8px) !important;
    overflow: visible !important;
    transform: none !important;
    scrollbar-width: none !important;
  }
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-toggle) {
    order: 10 !important;
    flex: 0 0 var(--mobile-fallback-danmu) !important;
    width: var(--mobile-fallback-danmu) !important;
    min-width: var(--mobile-fallback-danmu) !important;
    max-width: var(--mobile-fallback-danmu) !important;
  }

  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-settings-trigger) {
    order: 11 !important;
    flex: 0 0 var(--mobile-fallback-danmu-settings) !important;
    width: var(--mobile-fallback-danmu-settings) !important;
    min-width: var(--mobile-fallback-danmu-settings) !important;
    max-width: var(--mobile-fallback-danmu-settings) !important;
  }

  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-画质) {
    order: 12 !important;
    flex: 0 0 var(--mobile-fallback-quality) !important;
    width: var(--mobile-fallback-quality) !important;
    min-width: var(--mobile-fallback-quality) !important;
    max-width: var(--mobile-fallback-quality) !important;
  }

  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-倍速) {
    order: 13 !important;
    flex: 0 0 var(--mobile-fallback-rate) !important;
    width: var(--mobile-fallback-rate) !important;
    min-width: var(--mobile-fallback-rate) !important;
    max-width: var(--mobile-fallback-rate) !important;
  }

  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-字幕) {
    order: 14 !important;
    flex: 0 0 var(--mobile-fallback-subtitle) !important;
    width: var(--mobile-fallback-subtitle) !important;
    min-width: var(--mobile-fallback-subtitle) !important;
    max-width: var(--mobile-fallback-subtitle) !important;
  }

  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-setting),
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-landscape-fullscreen) {
    flex: 0 0 var(--mobile-fallback-icon) !important;
    width: var(--mobile-fallback-icon) !important;
    min-width: var(--mobile-fallback-icon) !important;
    max-width: var(--mobile-fallback-icon) !important;
  }
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-画质 .art-selector-value),
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-倍速 .art-selector-value),
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-字幕 .art-selector-value),
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right .art-control span) {
    display: inline-flex !important;
    align-items: center !important;
    justify-content: center !important;
    width: 100% !important;
  ...
  .player:not(.is-forced-landscape) .mobile-danmu-settings {
    ...
    width: min(342px, calc(100% - 24px)) !important;
    max-height: min(56vh, 430px) !important;
  }
}

@media (orientation: landscape) and (max-height: 640px) {
  .player .mobile-danmu-settings,
  .mobile-danmu-settings.is-mobile-portal.is-forced-landscape-portal {
    right: max(12px, env(safe-area-inset-right, 0px)) !important;
    bottom: max(70px, calc(env(safe-area-inset-bottom, 0px) + 70px)) !important;
    width: min(342px, calc(100% - 84px)) !important;
    max-height: calc(100% - 96px) !important;
  }
}

/* Final mobile player controls: use stable custom text entries instead of Artplayer text controls. */
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-quality),
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-rate),
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-subtitle),
.player.is-mobile-player :deep(.art-video-player .art-control-mobile-danmu-toggle),
...
.player.is-mobile-player :deep(.art-video-player .mobile-art-text-control) {
  display: inline-flex !important;
  align-items: center !important;
  justify-content: center !important;
  width: 100% !important;
...
@media (orientation: portrait), (max-width: 640px), (pointer: coarse), (hover: none) {
  .player.is-mobile-player:not(.is-forced-landscape) {
    --mobile-user-left-controls: clamp(100px, 27vw, 114px);
    --mobile-user-danmu: 26px;
    --mobile-user-danmu-settings: 38px;
    --mobile-user-quality: 40px;
    --mobile-user-rate: 26px;
    --mobile-user-subtitle: 34px;
    --mobile-user-icon: 26px;
  }
  ...
  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-left) {
    display: flex !important;
    flex: 0 0 var(--mobile-user-left-controls) !important;
    width: var(--mobile-user-left-controls) !important;
    min-width: 0 !important;
    max-width: var(--mobile-user-left-controls) !important;
    overflow: hidden !important;
  }
  ...
  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
    ...
    gap: 0 !important;
    min-width: 0 !important;
    max-width: calc(100% - var(--mobile-user-left-controls) - 6px) !important;
    overflow: visible !important;
    transform: none !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-toggle) {
    flex: 0 0 var(--mobile-user-danmu) !important;
    width: var(--mobile-user-danmu) !important;
    min-width: var(--mobile-user-danmu) !important;
    max-width: var(--mobile-user-danmu) !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-danmu-settings-trigger) {
    flex: 0 0 var(--mobile-user-danmu-settings) !important;
    width: var(--mobile-user-danmu-settings) !important;
    min-width: var(--mobile-user-danmu-settings) !important;
    max-width: var(--mobile-user-danmu-settings) !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-quality) {
    flex: 0 0 var(--mobile-user-quality) !important;
    width: var(--mobile-user-quality) !important;
    min-width: var(--mobile-user-quality) !important;
    max-width: var(--mobile-user-quality) !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-rate) {
    flex: 0 0 var(--mobile-user-rate) !important;
    width: var(--mobile-user-rate) !important;
    min-width: var(--mobile-user-rate) !important;
    max-width: var(--mobile-user-rate) !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-subtitle) {
    flex: 0 0 var(--mobile-user-subtitle) !important;
    width: var(--mobile-user-subtitle) !important;
    min-width: var(--mobile-user-subtitle) !important;
    max-width: var(--mobile-user-subtitle) !important;
  }

  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-setting),
  .player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-mobile-landscape-fullscreen) {
    flex: 0 0 var(--mobile-user-icon) !important;
    width: var(--mobile-user-icon) !important;
    min-width: var(--mobile-user-icon) !important;
    max-width: var(--mobile-user-icon) !important;
  }
}

@media (max-width: 370px) and (orientation: portrait) {
  .player.is-mobile-player:not(.is-forced-landscape) {
    --mobile-user-left-controls: clamp(88px, 25vw, 98px);
    --mobile-user-danmu: 23px;
    --mobile-user-danmu-settings: 33px;
    --mobile-user-quality: 36px;
    --mobile-user-rate: 23px;
    --mobile-user-subtitle: 30px;
    --mobile-user-icon: 23px;
  }
...
/* Phone hardening: keep the same compact control set in portrait as in landscape. */
.player.is-mobile-player:not(.is-forced-landscape) {
  --mobile-user-left-controls: clamp(90px, 25vw, 108px);
  --mobile-user-danmu: 24px;
  --mobile-user-danmu-settings: 34px;
  --mobile-user-quality: 36px;
  --mobile-user-rate: 24px;
  --mobile-user-subtitle: 30px;
  --mobile-user-icon: 24px;
}
...
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-left .art-control-time),
.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-control-time) {
  max-width: calc(var(--mobile-user-left-controls) - 44px) !important;
  font-size: 10px !important;
}

.player.is-mobile-player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
  max-width: calc(100% - var(--mobile-user-left-controls) - 4px) !important;
  overflow: visible !important;
}
...
@media (max-width: 370px) and (orientation: portrait) {
  .player.is-mobile-player:not(.is-forced-landscape) {
    --mobile-user-left-controls: clamp(84px, 24vw, 94px);
    --mobile-user-danmu: 22px;
    --mobile-user-danmu-settings: 32px;
    --mobile-user-quality: 34px;
...
/* Final phone hardening: keep the portrait bottom row as complete as landscape. */
@media (orientation: portrait), (max-width: 640px), (pointer: coarse), (hover: none) {
  .player:not(.is-forced-landscape) {
    --mobile-final-left: clamp(86px, 23vw, 112px);
    --mobile-final-danmu: 23px;
    --mobile-final-danmu-settings: 32px;
    --mobile-final-quality: 34px;
    --mobile-final-rate: 23px;
    --mobile-final-subtitle: 28px;
    --mobile-final-icon: 23px;
  }
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-control-time) {
    ...
    width: auto !important;
    min-width: 0 !important;
    max-width: calc(var(--mobile-final-left) - 42px) !important;
    overflow: hidden !important;
    padding-inline: 0 !important;
  ...
  .player:not(.is-forced-landscape) :deep(.art-video-player .art-controls-right) {
    ...
    gap: 0 !important;
    min-width: 0 !important;
    max-width: calc(100vw - var(--mobile-final-left) - 6px) !important;
    overflow: visible !important;
    margin-left: 0 !important;
    transform: none !important;
  }
...
.player .mobile-danmu-settings.is-mobile-portal.is-visible {
  display: block !important;
  visibility: visible !important;
  opacity: 1 !important;
  pointer-events: auto !important;
